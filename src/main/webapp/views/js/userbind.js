...
/*function userbind(){
   ...
   if(checkphoneNoFlag){
	 ...
	 $.ajax({
		type: "POST",  
		url: getUrl()+"/WechatLogin/userEverRegist.do", 
		data:{username:username},
	    success : function(data){
	    	if(data=="0"){   //用户没有绑定手机号码(校验验证码并判断是否绑定微信)
	    		$.ajax({
						   ...
						   if(result=="true"){   //手机验证码校验成功
							  ...
							  // var openId=$("#openId").val();//获取openId oOLN5w9uiHOpJ7CRzDZj37gwuutw
							   ...
							   $.ajax({
					    	        ...
					    	        async: false ,
					    	        data: {phoneNo:username,agentflag:agentflag},
					    	        success : function(result){
					    	    		if("绑定成功"===result){
					    	    			$("#Message").html("用户绑定成功.");
					    	        		$(".errorhei1").show();
	    		...
	    		$('#Message1').html("用户已经绑定手机号码，是否更改手机号码！");
		    	$('.errorhei2').show();
	    	}
	    }
		});
	   ...
	   if(checkphoneNoFlag){
    	   ...
    	   $.ajax({
			   ...
			   success : function(result){
				   if(result=="true"){   //手机验证码校验成功
					  // var openId=$("#openId").val();//获取openId oOLN5w9uiHOpJ7CRzDZj37gwuutw
					   var agentflag= $('input:radio[name="agent"]:checked').val();;
					   $.ajax({
			    	        url : getUrl()+"/WechatLogin/bindUser.do",
			    	        async: false ,
			    	        data: {phoneNo:username,agentflag:agentflag},
